...
function updateTraveloElementUI(obj)
{
    obj.next("span.custom-select").text(obj.find("option:selected").text());
}
...
tjq(window).load(function () {
    ...
    tjq(".go-to").live("click", function (e) {
	e.preventDefault();
	tjq("html,body").animate({scrollTop: tjq(tjq(this).data('to')).offset().top +60}, 500);
    });
	...
	tjq('<h1 class="logo navbar-brand"><a title="logo" href="javascript:void(0)"><img alt="" src="' + tjq('#PrincipalLogo').attr('src') + '"></a></h1>').insertBefore('.chaser .menu');
