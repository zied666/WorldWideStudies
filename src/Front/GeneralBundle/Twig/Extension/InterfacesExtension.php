...
class InterfacesExtension extends \Twig_Extension
{
    ...
    public function getFunctions()
    {
        return array(
            ...
            'convertCurrencyWithoutScale'=>new \Twig_Function_Method($this, 'convertCurrencyWithoutScale'),
    ...
    function convertCurrencyWithoutScale($amount, $from)
    {
        $session=$this->session->get('currency');
        if($from == $session['code'])
            return number_format($amount, $session['scale'], '.', ' ').' '.$from;
        ...
        {
            $converted=preg_replace("/[^0-9.]/", "", $converted[1]);
            return number_format($converted, 0, '.', ' ').' '.$session['code'];
        }
        return '';
    }
